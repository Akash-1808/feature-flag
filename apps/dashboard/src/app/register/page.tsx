...
import { Layers, ArrowRight, Loader2, Lock, Mail, User, Building2 } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiClient.post("/api/auth/sign-up/email", {
        name,
        email,
        password,
      });

      // 2. Create Organization via Better Auth plugin (triggers seedDefaultEnvironments hook!)
      const slug = orgName
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/(^-|-$)/g, "");

      await apiClient.post("/api/auth/organization/create", {
        name: orgName,
        slug: slug || `org-${Date.now()}`,
      });

      toast.success("Workspace created! Environments seeded. Redirecting...");
      router.push("/flags");
    } catch (error: any) {
